...
QueueFamilyIndices HelloTriangleApp::findQueueFamilies(const VkPhysicalDevice device)
{
	...
	for (const auto& queueFamily : queueFamilies) {
		...
		if (queueFamily.queueCount > 0 && presentSupport) //treated as different queues but likely to be in the same
...
bool HelloTriangleApp::checkDeviceExtensionSupport(const VkPhysicalDevice device)
{
	...
	return requiredExtensions.empty(); //pulling required extensions means : if empty then OK
}

SwapChainSupportDetails HelloTriangleApp::querySwapChainSupport(const VkPhysicalDevice device)
{
...
void HelloTriangleApp::createSwapChain()
{
	...
	auto imageCount = swapChainSupport.capabilities.minImageCount + 1; //triple buff
...
void HelloTriangleApp::drawFrame()
{
	...
	//more efficient way if program updates the app state
	/*void drawFrame() {
		updateAppState();
